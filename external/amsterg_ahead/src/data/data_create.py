...
def create_processed_data(stack=1,
    ...
    for game_run in tqdm(game_runs):
        ...
        gazes_fused_noop = fuse_gazes(images_,
                                      gazes,
                                      gaze_count=1)
        gazes = torch.stack(
...
def combine_processed_data(game):
    ...
    for datum in tqdm(data):
        ...
        for group in tqdm(groups[1:]):
            old_size = gaze_h5_file['combined'][datum].shape[0]
            new_size = old_size + gaze_h5_file[group][datum].shape[0]
            gaze_h5_file['combined'][datum].resize(new_size, axis=0)
            gaze_h5_file['combined'][datum][old_size:new_size] = gaze_h5_file[group][datum]
